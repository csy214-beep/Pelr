...
public:
    ...
    Csm::CubismMotionQueueEntryHandle StartMotion(const Csm::csmChar *group, Csm::csmInt32 no, Csm::csmInt32 priority,
                                                  Csm::ACubismMotion::FinishedMotionCallback onFinishedMotionHandler =
                                                          NULL, Csm::ACubismMotion::BeganMotionCallback
                                                  onBeganMotionHandler = NULL);
    ...
    */
    Csm::csmMap<Csm::csmString, Csm::ACubismMotion *> extraMotions; ///< 额外的动作列表
    Csm::csmMap<Csm::csmString, Csm::ACubismMotion *> extraExpressions; ///< 额外的表情列表
    /**
     * @brief 通过额外动作名称播放额外动作
     */
    CubismMotionQueueEntryHandle StartExtraMotion(const csmChar *motionName, csmInt32 priority,
                                                  ACubismMotion::FinishedMotionCallback onFinishedMotionHandler = NULL,
                                                  ACubismMotion::BeganMotionCallback onBeganMotionHandler = NULL);
    ...
    /**
     * @brief 检查动作是否存在（包括原有动作和额外动作）
     */
    bool HasMotion(const csmChar *motionName) const;

    /**
     * @brief 检查表情是否存在（包括原有表情和额外表情）
     */
...
private:
    ExtraFileManager *_extraFileManager;
